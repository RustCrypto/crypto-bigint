...
use crate::{
    uint::boxed::div_limb::{self, Reciprocal},
    BoxedUint, CheckedDiv, Limb, NonZero, Wrapping, ConstantTimeSelect,
};
...
impl BoxedUint {
    ...
    pub fn div_rem_limb_with_reciprocal(&self, reciprocal: &Reciprocal) -> (Self, Limb) {
        div_limb::div_rem_limb_with_reciprocal(self, reciprocal)
    }

    /// Computes `self` / `rhs`, returns the quotient (q) and remainder (r).
    pub fn div_rem_limb(&self, rhs: NonZero<Limb>) -> (Self, Limb) {
        div_limb::div_rem_limb_with_reciprocal(self, &Reciprocal::new(rhs))
    }
