...
impl UintRef {
    ...
    /// - if `shift >= Limb::BITS`.
    #[inline(always)]
    pub const fn shr_assign_limb_with_carry(&mut self, shift: u32, carry: Limb) -> Limb {
        let nz = Choice::from_u32_nz(shift);
        self.conditional_shr_assign_limb_nonzero(NonZero(nz.select_u32(1, shift)), carry, nz)
    }
