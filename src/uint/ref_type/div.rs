...
impl UintRef {
    ...
    pub const fn div_rem(&mut self, rhs: &mut Self) {
        ...
        y.wrapping_shl_assign(y.bits_precision() - ybits);
    ...
    /// Computes `self` % `rhs`, returning the remainder in `rhs`.
    ///
    /// This function operates in variable-time with respect to `rhs`. For a fixed divisor,
    /// it operates in constant-time. It will panic if the divisor is zero.
    pub const fn rem_vartime(&mut self, rhs: &mut Self) {
        let (x, y) = (self, rhs);
        let xsize = x.nlimbs();
        let ywords = y.bits_vartime().div_ceil(Limb::BITS) as usize;
        ...
        match (xsize, ywords) {
            ...
            (_, 1) => {
                // Perform limb division
                let rem_limb = x.rem_limb(y.0[0].to_nz().expect("zero divisor"));
                y.fill(Limb::ZERO);
                y.0[0] = rem_limb;
                return;
            }
    ...
    pub const fn div_rem_shifted(&mut self, mut x_hi: Limb, y: &mut Self, ywords: u32) {
        ...
        x_hi = Self::div_rem_large_shifted(x, x_hi, y, ywords, reciprocal);
    ...
    const fn div_rem_large_shifted(
        ...
        while xi > 0 {
            ...
            let mut quo = div3by2(x_hi.0, x_lo.0, x.0[xi - 1].0, &reciprocal, y.0[ysize - 2].0);
            ...
            x_lo = Limb::select(x.0[xi - 1], x_lo, done);
            xi -= 1;
        }
    ...
    const fn div_rem_large_vartime(&mut self, rhs: &mut Self) {
        ...
        // Perform the core division algorithm
        x_hi = Self::div_rem_large_shifted(x, x_hi, y, ysize as u32, reciprocal);
